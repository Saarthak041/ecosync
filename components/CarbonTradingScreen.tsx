...
const CarbonTradingScreen = () => {
  ...
  const initializeBlockchain = async () => {
    try {
      if (provider && signer && !testModeEnabled) {
        await carbonCreditService.initializeContract(provider, signer);
        await loadUserData();
      }
    } catch (error) {
      console.error('Error initializing blockchain:', error);
      Alert.alert('Error', 'Failed to initialize blockchain connection');
    }
  };
  ...
  const handleConnectWallet = async () => {
    try {
      console.log('🔗 Connect Wallet button clicked! Test mode:', testModeEnabled);
      ...
      if (testModeEnabled) {
        ...
        console.log('✅ Test wallet connection state set to true');
        ...
        setTimeout(() => {
          ...
          Alert.alert('Success', 'Test wallet connected successfully!', [
            { text: 'OK', onPress: () => {
              ...
              console.log('📊 Loading mock data after connection...');
              loadUserData();
            }}
          ]);
        }, 500);
      ...
      if (testModeEnabled) {
        ...
        setTimeout(() => {
          Alert.alert(
            ...
            `Transfer initiated! Transaction hash: ${mockTxHash.slice(0, 10)}...`,
            [{ text: 'OK', onPress: () => {
    ...
    try {
      ...
      if (testModeEnabled) {
        ...
        setTimeout(() => {
          Alert.alert(
            'Credits Retired',
            `Successfully retired ${quantity} credits! Transaction hash: ${mockTxHash.slice(0, 10)}...`,
            [{ text: 'OK', onPress: () => {
              setModalVisible(false);
              loadUserData();
            }}]
  ...
  const openActionModal = (credit: CarbonCredit, action: 'transfer' | 'retire') => {
    console.log('🎯 Action modal opened for:', action, 'credit:', credit.name);
...
const CreditCard = ({ credit, onTransfer, onRetire }: {
  ...
  <View style={styles.creditCard}>
    <View style={styles.creditHeader}>
      ...
      <Text style={styles.creditStatus}>{credit.status}</Text>
    </View>
