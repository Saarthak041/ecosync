...
export default function Trading() {
  ...
  const handleBuy = (credit: CarbonCredit) => {
    console.log('Buy button clicked for:', credit.company);
  ...
  const handleSell = (credit: CarbonCredit) => {
    console.log('Sell button clicked for:', credit.company);
    setSelectedCredit(credit);
    setActionType('sell');
    setQuantity('');
    setTotalAmount(0);
  ...
  const handleQuantityChange = (text: string) => {
    ...
    const price = selectedCredit?.price || 0;
    setTotalAmount(qty * price);
  };

  const executeTransaction = () => {
    if (!selectedCredit || !quantity || parseFloat(quantity) <= 0) {
      Alert.alert('Error', 'Please enter a valid quantity');
      return;
    }

    const action = actionType === 'buy' ? 'Purchase' : 'Sale';
    Alert.alert(
      'Transaction Successful!', 
      `${action} of ${quantity} ${selectedCredit.company} credits for ₹${totalAmount.toFixed(2)} completed successfully.`,
  ...
  const handleWallet = () => {
    ...
    alert('Wallet functionality - Connect your Web3 wallet to start trading');
  };
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>
              {actionType === 'buy' ? 'Buy' : 'Sell'} Carbon Credits
            </Text>
            
            {selectedCredit && (
              <>
                <Text style={styles.modalCreditInfo}>
                  {selectedCredit.company} • ₹{selectedCredit.price.toFixed(2)} per credit
                </Text>
                
                <View style={styles.modalInputContainer}>
                  <Text style={styles.modalLabel}>Quantity (Credits)</Text>
                  <TextInput
                ...
                <View style={styles.modalButtons}>
                  ...
                  <TouchableOpacity 
                    style={[styles.modalConfirmButton, actionType === 'sell' && styles.modalSellButton]} 
